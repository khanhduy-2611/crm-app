...
module.exports = function createDashboardRouter({ pool }) {
    ...
    router.get('/', async (req, res) => {
        ...
        const companyScope = scopedCanBoId === null
            ...
            : `AND EXISTS (
                ...
                WHERE scoped_vip.khach_hang_id = kh.id
                  AND scoped_vip.can_bo_quan_ly_id = $2
            )`;
        ...
        try {
            ...
            const companyParams = [...countParams, currentDay, limit, (khPage - 1) * limit];
            const vipParams = [...countParams, currentDay, limit, (vipPage - 1) * limit];
            const dayParamIndex = countParams.length + 1;
            const limitParamIndex = countParams.length + 2;
            const offsetParamIndex = countParams.length + 3;

            const [companyResult, vipResult, birthdayResult] = await Promise.all([
                pool.query(`
                    SELECT
                        kh.id,
                        kh.ten_khach_hang,
